...
# args = commandArgs(T)
# anno = args[1] # anno = "protrac_merged", anno = "protrac_pio", anno = "lietal_clusters"
...
# List files
fcounts.f <- list.files(here::here("strains_analysis/output/03-srna_process/fcounts_no_rmsk/", anno), "counts$", full.names = T, recursive = T)
...
# Write to file
out_counts_no_rmsk = here::here("strains_analysis/output/03-srna_process/rawcounts_no_rmsk/")
dir.create(out_counts_no_rmsk, F, T)
write.table(counts.table.full, paste0(out_counts_no_rmsk, "/", anno, ".fullRawcounts.tsv"), sep="\t", col.names=T, row.names=F, quote=F)
write.table(counts.table, paste0(out_counts_no_rmsk, "/", anno, ".rawcounts.tsv"), sep="\t", col.names=T, row.names=F, quote=F)
...
out_clusters_no_rmsk= here::here("strains_analysis/output/03-srna_process/clusters_no_rmsk/")
dir.create(out_clusters_no_rmsk, F, T)
write.table(clusters.filt, paste0(out_clusters_no_rmsk, "/", anno, ".mm10.gtf"), sep = "\t", quote = F, row.names = F, col.names = F)

}

### =============== DE ANALYSIS AND FISHER TEST =============== ###
for (anno in annots) {

# Define files
tev_f           <- "data/public/nellaker2012/allTEVs.shared.TEVID.mm9.mm10.tab" # TEVs from Nellaker et al., 2012, with mm10 coordinates
count_f         <- here::here(paste0("strains_analysis/output/03-srna_process/rawcounts_no_rmsk/", anno, ".rawcounts.tsv"))
clustercoords_f <- here::here(paste0("strains_analysis/output/03-srna_process/clusters_no_rmsk/", anno, ".mm10.gtf"))
t_f             <- "environment/sample_info.csv"
...
# Write all to file
outdir=here::here("strains_analysis/output/03-srna_process/fisher_DE_TEV_no_rmsk/",anno)
dir.create(outdir, F, T)
write.table(norm.counts, paste0(outdir,"/",anno,".normcounts.tsv"), quote = F, sep = "\t", row.names = F, col.names = T)
...
openxlsx::write.xlsx(TEVstatus_list, paste0(outdir,"/",anno,".TEVstatus.xlsx"), overwrite = T)
openxlsx::write.xlsx(DEGs_list, paste0(outdir,"/",anno,".DEGs.xlsx"), overwrite = T)
openxlsx::write.xlsx(TEV_DEG, paste0(outdir,"/",anno,".DEGs_TEVs.xlsx"), overwrite = T)
openxlsx::write.xlsx(fishers_res_list, paste0(outdir,"/",anno,".FishersRes.xlsx"), overwrite = T)
write.table(fishers_pvals, paste0(outdir,"/",anno,".FishersPvalues.tsv"), quote = F, sep = "\t", row.names = F, col.names = T)
